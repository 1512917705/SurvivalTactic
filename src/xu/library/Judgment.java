...
public final class Judgment implements IJudgmentLibrary, Serializable {
	...
	/*
	 * 所有的判定 采取坐标锁定的方式,但是可以适当增加其他类型锁定判定
	 * 
	 * 每个生物，有一个主锁定参数，为攻击法术和敌人判定，提供敌人坐标， 有视野*视野的生物记忆空间，可以记住视野内所有敌人的位置，
	 * 有x个永久记忆坐标，我也不知道有啥用，可能每个种族的生物可以记住它的出生地，每个种族会有势力范围
	 * 
	 * 敌人判定 视野内是否有敌人
	 * 
	 * 血量判定，饱腹值判定 自身血量是否高于n% 自身血量是否高于n 自身饱腹值是否大于n% 自身饱腹值是否大于n xy坐标敌人的血量是否高于n%
	 * xy坐标敌人的血量是否高于n xy坐标敌人的饱腹值是否高于n% xy坐标敌人的饱腹值是否高于n
	 * 
	 */
	...
	public static IJudgment getRandomNode() {
		return JUDGMENT.get(random.nextInt(JUDGMENT.size()));
	}
	...
	public enum FreeWillJud implements IJudgment {// 单例模式放内容
		...
		public boolean run(World w, Creature c, MainPar mainPar, FreeWill freePar) {
			if(mainPar==null||mainPar.getMainpar().length<3)
				return false;
