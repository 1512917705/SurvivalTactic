...
public final class WorldRun {
	 ...
	 * 计算所有可运行的生物
	 * 
	 * 如果玩家模式，以玩家为中心，进行游戏 如果自动运行，所有生物课运行（未完成）
	 *
	 * @param world
	 */
	public static void setAllCanRunCreature(World world, int type) {
		...
		if (type == 0) {
			...
			int x = world.getMax_x();
			int y = world.getMax_y();
			Creature[][] c;
			for (int i = 0; i < x; i++)//
			{
				for (int j = 0; j < y; j++)// 生物
				{
					 c = world.getCreatureMap();
					if (c[i][j] != null)
						world.getCreList().add((Creature) c[i][j]);
				}
			}
