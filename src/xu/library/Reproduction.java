...
public final class Reproduction {
	...
	public static void starVar(Creature c) {
		...
		// 节点变异增删改，第一次测试通过
		int MutantNodeLocation = 0;
		if(g.getTreeNodeList().size()>1)
			MutantNodeLocation = random.nextInt(g.getTreeNodeList().size());// 第几个节点变异
		...
		if (fOrN >= (FreeWillVar + GenomeVar + NodeVar)) {
			NodeDetailVar(c, g, tn, MutantNodeLocation);
			return;
		}
	}
	...
	public static void GenomeVar(Creature c, Genome g, int MutantGenomeLocation) {
		...
		if (type >= addGenomeVar) {
			...
			TreeNode treeNode1 = new TreeNode(node,
					Reproduction.getExample(node.getExample()),
					new FreeWillBehavior());
	...
	public static void NoidVar(Creature c, Genome g, TreeNode tn, int MutantNodeLocation) {
		...
		if (type < editNodeVar) {
			...
			TreeNode t = new TreeNode(Judgment.getRandomNode(),
					getExample(ij.getExample()), new FreeWillBehavior());
			g.getTreeNodeList().set(MutantNodeLocation, t);
		} else {
			if (type >= (editNodeVar + addNodeVar)&&g.getTreeNodeList().size()>3) {
				// 删除节点
	...
	public static void NodeDetailVar(Creature c, Genome g, TreeNode tn, int MutantNodeLocation) {
		...
		mainParVar(tn.getMainPar());
		/*if (type >= editValVar & tn.getMainPar() != null) {
			// 修改参数
			mainParVar(tn.getMainPar());
		} else {
			// 自由意志行为
			freeWillBehaviorVar(tn.getFreeWillBehavior());
		}*/
	}
	 ...
	 * @param i
	 */
	public static void mainParVar(MainPar mainPar) {// 某些节点没有数据，不需要变异
		// System.out.println(mainPar);
		int i = random.nextInt(mainPar.getMainpar().length);
		int[] par = mainPar.getMainpar();
		int[] max = mainPar.getMax();
		int[] min = mainPar.getMin();
		int[] flux = mainPar.getFlux();
		/*
		 * par[i] = par[i] + random.nextInt(par[i] + 1) * flux[i] / 100 //
		 * 在原值*(1-波动%)到原值*(1+波动%)中取随机值 - random.nextInt(par[i] - 1) * flux[i] /
		 * 100;
		 */
		par[i] = (int) (par[i] + Math.random() * (par[i] + parAdd) * flux[i] / 100
				- Math.random() * (par[i] - parLess) * flux[i] / 100);
	...
	public static void editfreeWillParVar(FreeWill freeWill, int i) {
		...
		/*
		 * 还有点问题 目前来说这个公式勉强能用就行，毕竟它“波动范围稳定”“可以变大”“可以变小” 数据的初始值和最终稳定区间完全无关
		 */
		par.set(i, (int) (par.get(i) + Math.random() * (par.get(i) + parAdd) - Math.random() * (par.get(i) - parLess)));
		/*
		 * if (par.get(i) >= -1 && par.get(i) <= 1) par.set(i, par.get(i) +
		 * random.nextInt(2)); if (par.get(i) > 1) par.set(i, par.get(i) +
		 * random.nextInt(par.get(i) + 1) - random.nextInt(par.get(i) - 1)); if
		 * (par.get(i) < -1) par.set(i, par.get(i) + random.nextInt(-par.get(i)
		 * + 1) - random.nextInt(-par.get(i) - 1));
		 */
	}
	...
	public static void freeWillBehaviorVar(FreeWillBehavior fwb) {
		int x = random.nextInt(addFreeWillBhVar + editFreeWillBhVar + delFreeWillBhVar);
		if (x < addFreeWillBhVar) {
		...
		} else if (x >= addFreeWillBhVar + editFreeWillBhVar & fwb.getA().length > 2) {
			// 删
			int[] a = fwb.getA();
			int[] a1 = new int[a.length - 1];// 新数组
			...
			fwb.getA()[i] = (int) (fwb.getA()[i] + Math.random() * (fwb.getA()[i] + parAdd)
					- Math.random() * (fwb.getA()[i] - parLess));
			if (fwb.getA()[i] > fwb.getMax())
				fwb.getA()[i] = fwb.getMax();
			if (fwb.getA()[i] < fwb.getMin())
				fwb.getA()[i] = fwb.getMin();

			fwb.getB()[i] = (int) (fwb.getB()[i] + Math.random() * (fwb.getB()[i] + parAdd)
					- Math.random() * (fwb.getB()[i] - parLess));
			if (fwb.getB()[i] > fwb.getMax())
				fwb.getB()[i] = fwb.getMax();
			if (fwb.getB()[i] < fwb.getMin())
				fwb.getB()[i] = fwb.getMin();

			fwb.getC()[i] = (int) (fwb.getC()[i] + Math.random() * (fwb.getC()[i] + parAdd)
					- Math.random() * (fwb.getC()[i] - parLess));
			if (fwb.getC()[i] > fwb.getMax())
				fwb.getC()[i] = fwb.getMax();
			if (fwb.getC()[i] < fwb.getMin())
				fwb.getC()[i] = fwb.getMin();

			fwb.getD()[i] = (int) (fwb.getD()[i] + Math.random() * (fwb.getD()[i] + parAdd)
					- Math.random() * (fwb.getD()[i] - parLess));
